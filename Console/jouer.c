...
void generate_monster()
{
    ...
    monster.defense = rand() % 11 + (monster.niveau - 1) * 10;
    switch (monster.niveau)
    {
        case 1:
            MIN = 25;
    ...
    while(monster.pv > 0)
    {
        ...
        else if (choix == 'b')
        {
            ...
            if (monster.pv > 0)
            {
                critical = critical_chance();
                if (!critical)
                    ecrire(L"Le %s a rat\x82 son attaque, il ne vous inflige donc aucun d\x82g\x83t\n\n", monster.type);
                else
                {
                    if (critical == 2)
                    {
                    ...
                    if (monster.attaque < hero.defense)
                        degats = 0;
                    ecrire(L"Le %s vous attaque avec une force %d et vous vous d%lcfendez avec une", monster.type, monster.attaque, 130);
                    ecrire(L"\nforce %d\n", hero.defense);
